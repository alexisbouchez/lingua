...
func TestCompileAndRun(t *testing.T) {
	...
	code, numLocals := Compile(fn, nil, nil, nil, NewStringTable(0), false, nil)
...
fn double(x: i32): i32 { add(x, x) }`
	...
	m := NewModule()
	CompileFile(f, m, "test.lingua", false)
...
func TestWASIHello(t *testing.T) {
	...
	CompileFile(f, m, "test.lingua", false) // auto-imports fd_write
