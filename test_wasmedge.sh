...
# Test Lingua compiler with WasmEdge runtime
echo "Testing Lingua with WasmEdge..."
echo
...
# Check wasmedge is available
if ! command -v wasmedge &> /dev/null; then
    echo "Error: wasmedge not found in PATH"
    exit 1
fi
...
output=$(wasmedge /tmp/hello.wasm 2>&1 | head -1)
...
output=$(wasmedge /tmp/fizzbuzz.wasm 2>&1 | grep -c "FizzBuzz" || true)
if [ "$output" -gt 0 ]; then
...
output=$(wasmedge /tmp/factorial.wasm 2>&1 | head -1)
...
output=$(wasmedge /tmp/countdown.wasm 2>&1 | head -1)
if [ "$output" == "10" ]; then
...
echo "All WasmEdge tests passed! ✓"
